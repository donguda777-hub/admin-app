...
/** 상단 업체 그룹(총 30칸). 업체명 바로 아래 행은 작업자 칸(이름·기준·차익 한 팝업) */
...
/** 업체명·색 톤 고정, 칸수는 프로젝트별 그리드 `companyWorkerSlotCounts`로 조절 */
...
/** table-layout:fixed + colgroup: 일·작업자·합계 가로 비율 (합계만 조금 넓게) */
const COL_DAY_PCT = 2.5;
const COL_SUM_PCT = 11;
const COL_WORKER_PCT =
  (100 - COL_DAY_PCT - COL_SUM_PCT) / WORKER_SLOT_COUNT;

/** 작업자별 금액 상한 (1억 미만, 천만 단위까지) */
function digitsOnly(s: string): string {
  return s.replace(/\D/g, "");
}

/** 휴대폰(01x…) 10자리 3-3-4, 11자리 3-4-4; 그 외는 최대 11자리까지 3-4-4 형태 */
...
/** 하단 공수 행 표시: 0이면 빈 문자열, 아니면 최대 소수 4자리까지 깔끔히 */
...
function ComputedMoneyFooterRow({
  ...
  masked?: boolean;
  /** L&N 행: 라벨을 버튼으로 표시(금액 보기 팝업) */
  labelButton?: { onClick: () => void };
}) {
  return (
...
export default function AdminMainScreen({
  ...
  /** 단가 팝업 대상(열기·저장 시 식별). 입력값은 workerRateDraft에만 둔다. */
  ...
  /** 급여·실급여·L&N 하단 행 마스킹 해제(세션만, localStorage 없음) */
  ...
  /** Supabase workers → 인적사항 등급별(조회 순서 유지) */
  ...
  /** 선택 연·월의 프로젝트별 공수표에서 계산(전체 요약표·합계 행). 그리드·이름·공수·L&N 반영 */
  ...
  return (
    <div className="flex h-[100dvh] max-h-[100dvh] min-h-0 flex-col overflow-hidden bg-slate-200 text-slate-900">
      ...
      {mainView === "timesheet" ? (
        <>
          {/* 연/월: 좌측 정렬, 드롭다운은 스크롤 밖(overflow 미클립) */}
